...
```
mirror/  ──build_site.py──►  dist/  ──make safe──►  CI ──► GitHub Pages
(raw source, not committed)  (generated)   (lyrics stripped)
```

`mirror/` is **not committed to this public repo** — it holds the byte-exact
original pages including the copyrighted song lyrics, which we do not
redistribute. It lives in a separate **private** mirror repo and is gitignored
here. Recreate it locally with `make mirror` (a Wayback crawl); CI checks it out
from the private repo via the `MIRROR_DEPLOY_KEY` secret. Only the lyric-stripped
`make safe` build is ever published.

## The golden rule: never hand-edit `mirror/` or `dist/`

- **`mirror/`** is the byte-for-byte archived source of truth. Treat it as
  read-only. It is only ever (re)written by `scripts/mirror.py` (`make mirror`),
  and it is gitignored — never commit it to this repo.
...
make mirror       # (re)download the raw archive into mirror/ — required once on
                  #   a fresh clone (mirror/ is not committed); ~30-45 min
